...
While task-aware approaches to modern compression techniques like neural compression have shown promising advancements in balancing the rate-utility trade-off, there remains a significant gap in analyzing their effects on time series data, specifically in vehicular contexts, where computational resources and bandwidth are often constrained. A lightweight, task-aware compression method for automotive time-series data is therefore needed. Based on the existing literature we can define three relevant research questions which this paper aims to answer.

\begin{itemize}
    \item \textbf{RQ1:} How does the use of tokenization as a transformation step before applying entropy coding change the architecture of the compression model?
    \item \textbf{RQ2:} What advantages and disadvantages does task-aware neural compression techniques offer when applied to time series data?
    \item \textbf{RQ3:} How does the use of neural-compression affect the rate-utility trade-off in the case of time-series data?
\end{itemize}
